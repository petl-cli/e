...
var playerSkipToNextTrackCmd = &cobra.Command{
	Use:   "skip-to-next-track",
	Short: "Skip To Next ",
	RunE:  withTelemetry(runPlayerSkipToNextTrack),
}

var playerSkipToNextTrackFlags struct {
	deviceId string
}

func init() {
	playerSkipToNextTrackCmd.Flags().StringVar(&playerSkipToNextTrackFlags.deviceId, "device-id", "", "")

	playerCmd.AddCommand(playerSkipToNextTrackCmd)
}

func runPlayerSkipToNextTrack(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "POST",
		Path:        httpclient.SubstitutePath("/me/player/next", pathParams),
	...
	if cmd.Flags().Changed("device-id") {
		req.QueryParams["device_id"] = fmt.Sprintf("%v", playerSkipToNextTrackFlags.deviceId)
	}
