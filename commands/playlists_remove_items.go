...
var playlistsRemoveItemsCmd = &cobra.Command{
	Use:   "remove-items",
	Short: "Remove Playlist Items ",
	RunE:  withTelemetry(runPlaylistsRemoveItems),
}

var playlistsRemoveItemsFlags struct {
	playlistId string
	tracks     []string
	snapshotId string
	body       string
...
func init() {
	playlistsRemoveItemsCmd.Flags().StringVar(&playlistsRemoveItemsFlags.playlistId, "playlist-id", "", "")
	playlistsRemoveItemsCmd.MarkFlagRequired("playlist-id")
	playlistsRemoveItemsCmd.Flags().StringSliceVar(&playlistsRemoveItemsFlags.tracks, "tracks", nil, "An array of objects containing [Spotify URIs](/documentation/web-api/concepts/spotify-uris-ids) of the tracks or episodes to remove. For example: `{ \"tracks\": [{ \"uri\": \"spotify:track:4iV5W9uYEdYUVa79Axb7Rh\" },{ \"uri\": \"spotify:track:1301WleyT98MSxVHPZCA6M\" }] }`. A maximum of 100 objects can be sent at once. ")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	playlistsRemoveItemsCmd.Flags().StringVar(&playlistsRemoveItemsFlags.snapshotId, "snapshot-id", "", "The playlist's snapshot ID against which you want to make the changes. The API will validate that the specified items exist and in the specified positions and make the changes, even if more recent changes have been made to the playlist. ")
	// Note: body fields are not MarkFlagRequired — --body JSON satisfies them too.
	playlistsRemoveItemsCmd.Flags().StringVar(&playlistsRemoveItemsFlags.body, "body", "", "Full request body as JSON (overrides individual flags)")

	playlistsCmd.AddCommand(playlistsRemoveItemsCmd)
}

func runPlaylistsRemoveItems(cmd *cobra.Command, args []string) error {
	...
	pathParams["playlist_id"] = fmt.Sprintf("%v", playlistsRemoveItemsFlags.playlistId)
	...
	if playlistsRemoveItemsFlags.body != "" {
		if err := json.Unmarshal([]byte(playlistsRemoveItemsFlags.body), &bodyMap); err != nil {
	...
	if cmd.Flags().Changed("tracks") {
		bodyMap["tracks"] = playlistsRemoveItemsFlags.tracks
	}
	if cmd.Flags().Changed("snapshot-id") {
		bodyMap["snapshot_id"] = playlistsRemoveItemsFlags.snapshotId
	}
	req.Body = bodyMap
