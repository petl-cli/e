...
var audiobooksGetSeveralCmd = &cobra.Command{
	Use:   "get-several",
	Short: "Get Several Audiobooks ",
	RunE:  withTelemetry(runAudiobooksGetSeveral),
}

var audiobooksGetSeveralFlags struct {
	ids    string
	market string
}

func init() {
	audiobooksGetSeveralCmd.Flags().StringVar(&audiobooksGetSeveralFlags.ids, "ids", "", "")
	audiobooksGetSeveralCmd.MarkFlagRequired("ids")
	audiobooksGetSeveralCmd.Flags().StringVar(&audiobooksGetSeveralFlags.market, "market", "", "")

	audiobooksCmd.AddCommand(audiobooksGetSeveralCmd)
}

func runAudiobooksGetSeveral(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "GET",
		Path:        httpclient.SubstitutePath("/audiobooks", pathParams),
	...
	if cmd.Flags().Changed("ids") {
		req.QueryParams["ids"] = fmt.Sprintf("%v", audiobooksGetSeveralFlags.ids)
	}
	if cmd.Flags().Changed("market") {
		req.QueryParams["market"] = fmt.Sprintf("%v", audiobooksGetSeveralFlags.market)
	}
