...
var albumsCheckSavedCmd = &cobra.Command{
	Use:   "check-saved",
	Short: "Check User's Saved Albums ",
	RunE:  withTelemetry(runAlbumsCheckSaved),
}

var albumsCheckSavedFlags struct {
	ids string
}

func init() {
	albumsCheckSavedCmd.Flags().StringVar(&albumsCheckSavedFlags.ids, "ids", "", "")
	albumsCheckSavedCmd.MarkFlagRequired("ids")

	albumsCmd.AddCommand(albumsCheckSavedCmd)
}

func runAlbumsCheckSaved(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("ids") {
		req.QueryParams["ids"] = fmt.Sprintf("%v", albumsCheckSavedFlags.ids)
	}
