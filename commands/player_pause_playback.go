...
var playerPausePlaybackCmd = &cobra.Command{
	Use:   "pause-playback",
	Short: "Pause Playback ",
	RunE:  withTelemetry(runPlayerPausePlayback),
}

var playerPausePlaybackFlags struct {
	deviceId string
}

func init() {
	playerPausePlaybackCmd.Flags().StringVar(&playerPausePlaybackFlags.deviceId, "device-id", "", "")

	playerCmd.AddCommand(playerPausePlaybackCmd)
}

func runPlayerPausePlayback(cmd *cobra.Command, args []string) error {
	...
	req := &httpclient.Request{
		Method:      "PUT",
		Path:        httpclient.SubstitutePath("/me/player/pause", pathParams),
	...
	if cmd.Flags().Changed("device-id") {
		req.QueryParams["device_id"] = fmt.Sprintf("%v", playerPausePlaybackFlags.deviceId)
	}
