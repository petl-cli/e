...
var tracksCheckSavedCmd = &cobra.Command{
	Use:   "check-saved",
	Short: "Check User's Saved Tracks ",
	RunE:  withTelemetry(runTracksCheckSaved),
}

var tracksCheckSavedFlags struct {
	ids string
}

func init() {
	tracksCheckSavedCmd.Flags().StringVar(&tracksCheckSavedFlags.ids, "ids", "", "")
	tracksCheckSavedCmd.MarkFlagRequired("ids")

	tracksCmd.AddCommand(tracksCheckSavedCmd)
}

func runTracksCheckSaved(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("ids") {
		req.QueryParams["ids"] = fmt.Sprintf("%v", tracksCheckSavedFlags.ids)
	}
