...
var playlistsUsersUnfollowCmd = &cobra.Command{
	Use:   "users-unfollow",
	Short: "Unfollow Playlist ",
	RunE:  withTelemetry(runPlaylistsUsersUnfollow),
}

var playlistsUsersUnfollowFlags struct {
	playlistId string
}

func init() {
	playlistsUsersUnfollowCmd.Flags().StringVar(&playlistsUsersUnfollowFlags.playlistId, "playlist-id", "", "")
	playlistsUsersUnfollowCmd.MarkFlagRequired("playlist-id")

	playlistsCmd.AddCommand(playlistsUsersUnfollowCmd)
}

func runPlaylistsUsersUnfollow(cmd *cobra.Command, args []string) error {
	...
	pathParams["playlist_id"] = fmt.Sprintf("%v", playlistsUsersUnfollowFlags.playlistId)
